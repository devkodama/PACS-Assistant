...
ClickId(WebView, id) {
    ...
    switch id {
        ...
        ; Window save, restore, and reset buttons
        case "button-remember":
            DispatchQueue.Push(GUISaveWindowPositions)
        case "button-restore":
            DispatchQueue.Push(GUIRestoreWindowPositions)
        case "button-resetlayout":
            DispatchQueue.Push(GUIResetWindowPositions)
...
; Remember current window positions and save to settings file
...
; Restore saved window positions from currently selected layout. Does not read from .ini file.
...
; Reset layout to autogenerated default layout and save to settings file.
GUIResetWindowPositions(*) {
    global Layout
    static running := false
    ...
    GUIStatus("Resetting layout...")

    Layout[Setting["Layout"].value].ResetToDefault()
    Layout[Setting["Layout"].value].Save(Setting["Layout"].value)        ; save to .ini file

    GUIStatus("Reset (Press Restore windows to move windows)")
